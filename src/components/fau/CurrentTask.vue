        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            <el-button type="primary"
                       ...
                       @click="showGoodsApplyVisible(row)">申请物资
            </el-button>
            <el-button type="primary"
                       @click="acceptTask(row.id)"
                       v-if="row.status === 'ready'">接受
            </el-button>
            <el-button @click="rejectTask(row.id)" v-if="row.status === 'ready'">放弃</el-button>
            <el-button type="success"
                       @click="showFinishVisible(row.id)"
                       v-if="row.status === 'fixing'">完成
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  export default {
    ...
    methods: {
      initTask() {
        this.getRequest("/fault/task/").then(resp => {
          this.tasks = resp;
        })
      },
